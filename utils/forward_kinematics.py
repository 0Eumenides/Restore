...
def fkl_torch_no(angles, parent, offset, rotInd, expmapInd):
    ...
    # 创建一个新的 R 和 p3d 用于存储计算结果，避免就地操作
    new_R = R.clone()
    new_p3d = p3d.clone()

    for i in np.arange(1, j_n):
        if parent[i] > 0:
            # 更新 new_R 和 new_p3d，而不是原来的 R 和 p3d
            new_R[:, i, :, :] = torch.matmul(R[:, i, :, :], R[:, parent[i], :, :])
            new_p3d[:, i, :] = torch.matmul(p3d[0, i, :], R[:, parent[i], :, :]) + p3d[:, parent[i], :]
