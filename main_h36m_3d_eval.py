...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (p3d_h36, label) in enumerate(data_loader):
        # print(i)
        ...
        p3d_out_all = net_pred(p3d_src, label=label, input_n=in_n, output_n=10)

        p3d_out_all = p3d_out_all[:, seq_in:].transpose(1, 2).reshape([batch_size, 10, -1])[:, :out_n]
