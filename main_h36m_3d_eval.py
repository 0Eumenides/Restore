...
from  main_h36m_3d_pre import getMask
...
def main(opt):
    ...
    net_restore.cuda()
    ckpt = torch.load("/code/dth/Restore/checkpoint/20240328/V3_184600/ckpt_best.pth.tar")
    ...
    for i, act in enumerate(acts):
        ...
        ret_test = run_model(net_pred, net_restore,is_train=3, data_loader=test_loader, opt=opt)
...
def run_model(net_pred, net_restore, is_train=0, data_loader=None, epo=1, opt=None):
    net_pred.eval()
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        # input_gcn, restore_loss = net_restore(src, src, mask, start)
        p3d_src[:,:10]= src.view(-1, in_n, 66)
        p3d_out_all = net_pred(p3d_src, input_n=in_n, output_n=10)

        p3d_out_all = p3d_out_all[:, in_n:].transpose(1, 2).reshape([batch_size, 10, -1])[:, :out_n]
