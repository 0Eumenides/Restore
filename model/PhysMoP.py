...
class Fusion(nn.Module):
    ...
    def forward(self, motion_pred_data, motion_pred_physics, motion_feats, t):
        ...
        motion_fusion_feats = torch.cat([motion_fusion_feats_data, motion_fusion_feats_physics, motion_feats],
                                        dim=2) + t.unsqueeze(2)
...
class Regression(nn.Module):
    ...
    def forward(self, motion_input, gt_motion, mode, fusion=True):
        ...
        if self.data:
            ...
            motion_pred_data[:, config.hist_length:] = self.motion_fc_out(motion_feats) + motion_pred_data[:,
                                                                                          config.hist_length - 1:config.hist_length]
        ...
        for t in range(config.total_length - 3):
            ## physics gt history
            # 如果在训练，就用真实值结合加速度来求下一帧
            if self.physics and mode == 'train':
                pred_q_ddot_physics_gt[:, t + 1] = self.physics_forward(motion_feats_all.clone(), gt_motion[:, t:t + 3],
                                                                        B, D)
                motion_pred_physics_gt[:, t + 3] = 2 * gt_motion[:, t + 2] - gt_motion[:,
                                                                             t + 1] + pred_q_ddot_physics_gt[:,
                                                                                      t + 1].clone() * (1 / 25) ** 2

            if t > config.hist_length - 4:
                ## physics pred history
                # 如果在训练，就用gt值来作为pred值，因为这样可以加速收敛
                ...
                # 否则，也就是在测试，当然不能用到gt值，全部都要由加速度递推过来
                # else:
                pred_q_ddot_physics_pred[:, t + 1] = self.physics_forward(motion_feats_all.clone(),
                                                                          motion_pred_physics_pred[:, t:t + 3], B,
                                                                          D)
                motion_pred_physics_pred[:, t + 3] = 2 * motion_pred_physics_pred[:,
                                                         t + 2] - motion_pred_physics_pred[:,
                                                                  t + 1] + pred_q_ddot_physics_pred[:,
                                                                           t + 1].clone() * (1 / 25) ** 2
        ## fusion
        if fusion:
            time_idx = torch.arange(config.pred_length).float().to(motion_input.device).expand(B,
                                                                                               -1) / config.pred_length
            weight_t = torch.tanh(self.fusion_net(motion_pred_data[:, config.hist_length:].clone().detach(),
                                                  motion_pred_physics_pred[:, config.hist_length:].clone().detach(),
                                                  motion_feats.clone().detach(), time_idx.clone().detach())) ** 2

            for t in range(config.total_length - 3):
                if t > config.hist_length - 4:
                    pred_q_ddot_physics_pred_fusion = self.physics_forward(motion_feats_all.clone(),
                                                                           motion_pred_fusion[:, t:t + 3], B, D)
                    motion_pred_fusion_t = 2 * motion_pred_fusion[:, t + 2] - motion_pred_fusion[:, t + 1] + \
                                           pred_q_ddot_physics_pred_fusion.clone() * (1 / 25) ** 2

                    motion_pred_fusion[:, t + 3] = (1 - weight_t[:,
                                                        t + 3 - config.hist_length]) * motion_pred_fusion_t + weight_t[
                                                                                                              :,
                                                                                                              t + 3 - config.hist_length] * motion_pred_data[
                                                                                                                                            :,
                                                                                                                                            t + 3]
